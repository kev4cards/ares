...
PeripheralSettings& peripheralSettings = settingsWindow.peripheralSettings;
...
auto Settings::process(bool load) -> void {
  ...
  bind(real,    "Peripheral/StickRange", peripheral.stickRange);
...
SettingsWindow::SettingsWindow() {
  onClose([&] {
    ...
    peripheralSettings.setVisible(false);
  ...
  panelList.append(ListViewItem().setText("Peripheral").setIcon(Icon::Device::Joypad));
  ...
  panelContainer.append(peripheralSettings, Size{~0, ~0});
...
auto SettingsWindow::eventChange() -> void {
  ...
  if(auto item = panelList.selected()) {
    ...
    if(item.text() == "Peripheral"  ) found = true, peripheralSettings.setVisible();
