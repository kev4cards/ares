auto StickSettings::construct() -> void {
  ...
  outputStyleLabel.setText("Output Style:").setToolTip(
    "Each output style is either a circle, a shape cut out of a circle,\n"
  ...
  maxOutputReducerOneFactorLabel.setText("Max Output Reducer 1 Factor:").setToolTip(
    "Reduces maximum output by set percentage. \n\n"
  ...
  customMaxOutputLabel.setText("Custom Max Output:").setToolTip(
    ...
    "Percentage refers to physical limit of system's native stick assembly.\n"
    "A value of 127 represents the true limit of the system.\n\n"
    "Only applies to custom output styles."
  );
  customMaxOutputValue.setAlignment(0.5).setToolTip(customMaxOutputLabel.toolTip());
  customMaxOutputSlider.setLength(128).setPosition(settings.stick.customMaxOutput);
  customMaxOutputSlider.onChange([&] {
    settings.stick.customMaxOutput = customMaxOutputSlider.position();
    customMaxOutputValue.setText({customMaxOutputSlider.position(), " (", customMaxOutputSlider.position() / 85.0 * 100.0, "%)"}); //Important to include percentage like this? Reflects physical limitation by gate of the controller but not electrical limitation
    updateSlider();
  }).doChange();
  customMaxOutputSpacer.setColor({192, 192, 192});

  deadzoneShapeLayout.setPadding(12_sx, 6_sy);
  deadzoneShapeLabel.setText("Deadzone Shape:");
  ...
  rangeNormalizedInflectionPointLabel.setText("Inflection Point:").setToolTip(
    "Set the relative distance with respect to the range of deadzone to max output\n"
    "where the curve changes behavior."
  );
  ...
  notchLengthFromEdgeLabel.setText("Notch Length from Edge:").setToolTip(
    "Set the notch length from circular edge of analog stick.\n"
...
auto StickSettings::updateSlider() -> void {
  ...
  auto outerDeadzoneRadiusMax = 0.0;
  ...
  if(settings.stick.outputStyleChoice == "Custom Octagon (Morphed)") {
    ...
    outerDeadzoneRadiusMax = maxOutputMultiplier * 85.0;
  } else if(settings.stick.outputStyleChoice == "Custom Octagon (Virtual)") {
    diagonalMax = maxOutputMultiplier * 69.0;
    cardinalMax = maxOutputMultiplier * 85.0;
    outerDeadzoneRadiusMax = sqrt(2.0) * (diagonalMax / cardinalMax * (cardinalMax - configuredInnerDeadzone) + configuredInnerDeadzone);
    ...
    diagonalMax = 127.0;
    cardinalMax = 127.0;
    outerDeadzoneRadiusMax = 127.0 * sqrt(2.0);
  ...
  if(settings.stick.outputStyleChoice == "Octagon (Virtual) (Default)" || settings.stick.outputStyleChoice == "Custom Octagon (Virtual)" || settings.stick.outputStyleChoice == "Square (Maximum Virtual)") {
    deadzoneSizeHint.setText({"Virtual: ", settings.stick.deadzoneSize / cardinalMax * 100.0, "%     ", "Real: ",  settings.stick.deadzoneSize / outerDeadzoneRadiusMax * 100.0, "%"}).setFont(Font().setSize(7.0)).setForegroundColor(SystemColor::Sublabel);
  } else {
    deadzoneSizeHint.setText({"Virtual: N/A     ", "Real: ", settings.stick.deadzoneSize / outerDeadzoneRadiusMax * 100.0, "%"}).setFont(Font().setSize(7.0)).setForegroundColor(SystemColor::Sublabel);
  }
  auto inflectionPoint = settings.stick.rangeNormalizedInflectionPoint * (cardinalMax - configuredInnerDeadzone) + configuredInnerDeadzone;
  auto b = 1.0;
  auto c = b * (log(1.0 - cos(Math::Pi * (inflectionPoint - configuredInnerDeadzone) / (cardinalMax - configuredInnerDeadzone))) - log(2.0)) / log((inflectionPoint - configuredInnerDeadzone) / (cardinalMax - configuredInnerDeadzone));
  ...
  if(settings.stick.responseCurveChoice == "Aggressive to Relaxed" || settings.stick.responseCurveChoice == "Aggressive to Linear" || settings.stick.responseCurveChoice == "Linear to Aggressive") {
    maxA = Math::Pi * 1e-09 / (Math::Pi * 1e-09 - c * (cardinalMax - configuredInnerDeadzone) * tan(Math::Pi * 1e-09 / (2 * (cardinalMax - configuredInnerDeadzone))));
  ...
  auto diagonalInput = outerDeadzoneRadiusMax * sqrt(2.0) / 2.0;
  auto diagonalOutput = 0.0;

  if(settings.stick.responseCurveChoice == "Linear (Default)") {
    diagonalOutput = configuredProportionalSensitivity * (diagonalInput - configuredInnerDeadzone) * cardinalMax / (cardinalMax - configuredInnerDeadzone);
    ...
    diagonalOutput = configuredProportionalSensitivity * pow(((diagonalInput - configuredInnerDeadzone) / (cardinalMax - configuredInnerDeadzone)), (a / b)) * cardinalMax * pow((sin(((diagonalInput - configuredInnerDeadzone) / (cardinalMax - configuredInnerDeadzone)) * Math::Pi / 2.0)), (2.0 * (b - a) / c));
  ...
  if((diagonalOutput < diagonalMax) && !(settings.stick.outputStyleChoice == "Octagon (Morphed)" || settings.stick.outputStyleChoice == "Square (Maximum Morphed)" || settings.stick.outputStyleChoice == "Custom Octagon (Morphed)")) {
    if(settings.stick.responseCurveChoice == "Relaxed to Aggressive" || settings.stick.responseCurveChoice == "Relaxed to Linear") {
      responseStrengthHint.setText("Note: Reduced diagonal output. Lower the inflection point, raise sensitivity, decrease deadzone, and/or increase max output for full diagonal output.").setFont(Font().setSize(7.0)).setForegroundColor(SystemColor::Sublabel);
    } else if(settings.stick.responseCurveChoice == "Aggressive to Relaxed" || settings.stick.responseCurveChoice == "Linear to Aggressive") {
      responseStrengthHint.setText("Note: Reduced diagonal output. Raise the inflection point, raise sensitivity, decrease deadzone, and/or increase max output for full diagonal output.").setFont(Font().setSize(7.0)).setForegroundColor(SystemColor::Sublabel);
      ...
      responseStrengthHint.setText("Note: Reduced diagonal output. Raise sensitivity, decrease deadzone, and/or increase max output for full diagonal output.").setFont(Font().setSize(7.0)).setForegroundColor(SystemColor::Sublabel);
    }
  } else {
